...
ESP8266/ESP8285 firmware that bridges MagicHome RGBW LED controllers to Apple HomeKit. Control your RGB+White LED strips with the Home app, Siri, or an IR remote — with full state synchronization between both.

## Features

- **Apple HomeKit** lightbulb accessory with on/off, brightness, hue, and saturation
- **IR remote control** via NEC protocol (compatible with standard MagicHome remotes)
...
- ESP8266 or ESP8285 module (e.g. MagicHome controller board)
- RGBW LED strip
- IR receiver (e.g. VS1838B) — optional, for remote control
...
Adjust the port (`/dev/ttyUSB0`) to match your system. On macOS this is typically `/dev/cu.usbserial-*`.
